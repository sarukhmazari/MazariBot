# 〔 𝗭𝗢𝗫𝗘𝗥  𝗕𝗢𝗧 〕— Developer & Architecture Reference

This document serves as the comprehensive architectural and development reference for the **ZOXER BOT**. It outlines the project's folder structure, initialization flow, session management, event handling pipeline, data model, admin dashboard API, and configuration details.

---

## 📌 Project Overview
**ZOXER BOT** is a high-performance, multi-session WhatsApp bot system built on top of the `@whiskeysockets/baileys` library (version `7.0.0-rc.9`). 
...
| :--- | :--- | :--- |
| `BOT_NAME` | Display name of the bot | `"〔 𝗭𝗢𝗫𝗘𝗥  𝗕𝗢𝗧 〕"` |
| `BOT_OWNER` | Owner's team name | `"ZOXER TEAM"` |
...
| `BOT_PACKNAME` | Default packname for generated stickers | `"ZOXER BOT"` |
| `BOT_AUTHOR` | Default author name for generated stickers | `"ZOXER TEAM"` |
...
| `ADMIN_API_KEY` | API Key to secure dashboard HTTP endpoints | `"zoxer_secret_key"` |
...
```bash
docker build -t zoxerbot .
docker run -d -p 7860:7860 --env-file .env zoxerbot
```
...
```bash
npm install pm2 -g
pm2 start index.js --name "zoxerbot" --exp-backoff-restart-delay=1000
```
To run with garbage collection enabled:
```bash
pm2 start index.js --node-args="--expose-gc --max-old-space-size=900" --name "zoxerbot"
```
