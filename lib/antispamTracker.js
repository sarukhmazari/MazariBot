...
function loadTracker() {
    if (memoryTracker) return memoryTracker;
    try {
        ...
        memoryTracker = JSON.parse(fs.readFileSync(TRACKER_PATH, 'utf8'));
        return memoryTracker;
    } catch (e) {
...
function saveTracker(data) {
    ...
    if (!saveTimeout) {
        saveTimeout = setTimeout(() => {
            try {
                fs.writeFileSync(TRACKER_PATH, JSON.stringify(memoryTracker, null, 2));
            } catch (e) {
