  ...
  for (const [phone, sock] of sessions.entries()) {
    if (sessionStates.get(phone) === 'CONNECTED') {
      try {
        ...
        if (errStr.includes('xwa2_newsletter_join_v2') || errStr.includes('ACTIVE')) {
          successCount++; // WhatsApp actually joined successfully but Baileys failed to parse it
        } else {
          console.error(`Failed to follow channel for ${phone}`, errStr);
        }
      }
    }
  }
