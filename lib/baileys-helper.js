...
async function followChannel(inviteCodeOrJid) {
  ...
  if (jid.includes('whatsapp.com/channel/')) {
    const code = jid.split('whatsapp.com/channel/')[1].split('/')[0];
    ...
    try {
      const metadata = await firstSession.newsletterMetadata("invite", code);
  ...
  for (const [phone, sock] of sessions.entries()) {
    if (sessionStates.get(phone) === 'CONNECTED') {
      try {
        ...
        console.error(`Failed to follow channel for ${phone}`, err);
      }
    }
  }
  
  // Optionally, we could add it to settings.js to persist, but for now we follow on all active sessions.
  return { jid, successCount, totalSessions: Array.from(sessions.keys()).filter(p => sessionStates.get(p) === 'CONNECTED').length };
}

module.exports = { initSession, requestPairingCode, terminateSession, runAutoFollow, followChannel, question, sessions, pairingCodesStore, sessionStates };
