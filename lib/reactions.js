...
// List of emojis for auto-reactions
...
// Function to add reaction to any message (not just commands)
async function addCommandReaction(sock, message) {
    try {
        if (!isAutoReactionEnabled || !message?.key?.id) return;
        
        // Skip reactions to bot's own messages to avoid spam
        if (message.key.fromMe) return;
        
        // Skip reactions to status messages
...
async function handleAreactCommand(sock, chatId, message, isOwner) {
    try {
        ...
        if (action === 'on') {
            ...
            await sock.sendMessage(chatId, { 
                text: '✅ Auto-reactions have been enabled globally!\n\n🤖 The bot will now react to every message with random emojis.',
                quoted: message
            });
        } else if (action === 'off') {
            ...
            await sock.sendMessage(chatId, { 
                text: '✅ Auto-reactions have been disabled globally!\n\n🤖 The bot will no longer react to messages automatically.',
                quoted: message
            });
        } else {
            const currentState = isAutoReactionEnabled ? 'enabled' : 'disabled';
            const statusEmoji = isAutoReactionEnabled ? '✅' : '❌';
            await sock.sendMessage(chatId, { 
                text: `${statusEmoji} Auto-reactions are currently ${currentState} globally.\n\nUse:\n.areact on - Enable auto-reactions\n.areact off - Disable auto-reactions\n\n💡 When enabled, the bot reacts to every message with random emojis!`,
                quoted: message
            });
        }
    } catch (error) {
